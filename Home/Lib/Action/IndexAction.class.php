...
class IndexAction extends Action {
    ...
    public function doReg(){
		...
		$whzp_userphone = cookie('whzp_userphone');
    	if($user_data){
    		session('USERPHONE',$user_data['userphone']);
    		echo json_encode(array('status'=>'ok'));
    		exit;
    	}elseif(($whzp_userphone!=null) && ($whzp_userphone!=$userphone)){
			echo json_encode(array('status'=>'ng','message'=>'您输入的手机号码与先前不一致!'));
    		exit;
		}	
		else{
    		//添加用户
			cookie('whzp_userphone',$userphone,3600*24*356);
